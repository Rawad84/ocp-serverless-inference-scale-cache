"""
Handler v3 — weighted ensemble + model agreement flag.

v3 changes over v2:
- sklearn pipeline models (joblib) are weighted 2x vs the linear npz model.
- Adds model_agreement bool: True when all scores are on the same side of 0.5.
- Returns per-model scores breakdown for debugging.
"""
...
def _run_inference(models: dict) -> dict:
    ...
    sklearn (joblib) models: weight = 2.0
    linear (npz) models:     weight = 1.0
    Adds model_agreement: True when all models predict same class (>0.5 or <0.5).
    """
    ...
    for filename, obj in models.items():
        if filename.endswith(".npz"):
            ...
            score = float(np.tanh(np.dot(W[0], x) / 100))
        elif filename.endswith(".joblib"):
            weight = 2.0
            clf = obj
            try:
        ...
        total_weight  += weight
        scores_detail[filename] = round(score, 4)

    if total_weight == 0:
        return {"prediction": 0.0, "model_agreement": False, "scores": {}}

    prediction = round(weighted_sum / total_weight, 4)
    all_scores = list(scores_detail.values())
    # agreement: all models predict same side of 0.5
    model_agreement = all(s > 0.5 for s in all_scores) or all(s <= 0.5 for s in all_scores)
